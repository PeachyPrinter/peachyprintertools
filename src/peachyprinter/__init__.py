...
if os.name == 'nt':
    ...
    if os.environ.get('PEACHY_API_DLL_PATH'):
        ...
        logging.warning("Loading usb dll via PEACHY_API_DLL_PATH")
    else:
        try:
            ...
            logging.warning("Loading usb dll via package resources")
        except Exception:
            ...
            logging.warning("Loading usb dll via relitive path")
    dll_path = os.path.join(dep_path, 'libusb-1.0.dll')

    logging.warning("Attempting Loading usb dll from: %s" % dll_path)
    try:
