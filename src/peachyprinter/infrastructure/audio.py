            ...
            for sample_rate in self._supported_rates:
                if input_device['maxInputChannels'] >= 1:
                    for depth in self._get_depths_for_rate(pa, input_device_id, sample_rate, 'input'):
                        inputs.append(
                            {'sample_rate': sample_rate, 'depth': depth})
...
class AudioWriter(DataWriter):
    ...
    def __init__(self, sample_rate, bit_depth, buffer_divisor=2):
        ...
        logging.info("Audio Writer started with sample rate: %s, bit depth %s" % (
            self._sample_rate, self._bit_depth))
        try:
            self._configuration_supported(self._sample_rate, self._format)
        except:
            logging.error("Audio configuration not supported; Sample Rate: %s; Sample Bits: %s " % (
                self._sample_rate, self._bit_depth))
            raise Exception("Audio configuration not supported; Sample Rate: %s; Sample Bits: %s " % (
                self._sample_rate, self._bit_depth))
